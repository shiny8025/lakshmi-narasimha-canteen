...
export default function DeliveryMenuPage() {
  ...
  const menuItems = getMenuByMode("delivery")
  const extraCharge = getExtraCharge("delivery")
  ...
  const goToCart = () => {
    localStorage.setItem("cart", JSON.stringify({ items: cart, mode: "delivery", extraCharge }))
    router.push("/cart")
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-serif font-black text-primary">Delivery Menu</h1>
                <p className="text-sm text-muted-foreground">Delivery charge: ₹{extraCharge}</p>
              </div>
            </div>
            {getTotalItems() > 0 && (
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">No items available in this category for delivery.</p>
          </div>
        )}
      </div>
    </div>
