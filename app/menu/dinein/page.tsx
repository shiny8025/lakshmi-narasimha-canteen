...
export default function DineInMenuPage() {
  ...
  const menuItems = getMenuByMode("dinein")
  const extraCharge = getExtraCharge("dinein")
  ...
  const goToCart = () => {
    localStorage.setItem("cart", JSON.stringify({ items: cart, mode: "dinein", extraCharge }))
    router.push("/cart")
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-serif font-black text-primary">Dine-In Menu</h1>
                <p className="text-sm text-muted-foreground">No service charge</p>
              </div>
            </div>
            {getTotalItems() > 0 && (
